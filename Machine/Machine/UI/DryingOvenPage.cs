...
namespace Machine
{
    public partial class DryingOvenPage : FormEx
    {
        ...
        private bool UnBindPallet;                      // 解绑夹具
        #endregion
        ...
        private void CreateParameterListView()
        {
            ...
            this.listViewParameter.Columns.Add("参数", width * 4, HorizontalAlignment.Center);
            ...
            item = new ListViewItem();
            item.Text = "12)真空加热总时间";
            item.SubItems.Add("0");
            this.listViewParameter.Items.Add(item);

            item = new ListViewItem();
            item.Text = "13)A状态真空最大值";
            item.SubItems.Add("0");
            this.listViewParameter.Items.Add(item);

            item = new ListViewItem();
            item.Text = "14)A状态真空保压时间";
            item.SubItems.Add("0");
            this.listViewParameter.Items.Add(item);

            item = new ListViewItem();
            item.Text = "15)B状态抽真空报警时间";
            item.SubItems.Add("0");
            this.listViewParameter.Items.Add(item);

            item = new ListViewItem();
            item.Text = "16)B状态真空压力";
            item.SubItems.Add("0");
            this.listViewParameter.Items.Add(item);

            item = new ListViewItem();
            item.Text = "17)B状态真空最大值";
            item.SubItems.Add("0");
            this.listViewParameter.Items.Add(item);

            item = new ListViewItem();
            item.Text = "18)B状态真空保压时间";
            item.SubItems.Add("0");
            this.listViewParameter.Items.Add(item);

            item = new ListViewItem();
            item.Text = "19)呼吸充干燥气保持时间";
            item.SubItems.Add("0");
            this.listViewParameter.Items.Add(item);

            item = new ListViewItem();
            item.Text = "20)呼吸充干燥气压力";
            item.SubItems.Add("0");
            this.listViewParameter.Items.Add(item);
        ...
        private void CreateStateListView()
        {
            ...
            this.listViewState.Columns.Add("5层", width * 30, HorizontalAlignment.Center);
            // 设置表格高度
            ImageList iList = new ImageList();
        ...
        private void buttonErrorReset_Click(object sender, EventArgs e)
        {
            ...
            for (int idx = 0; idx < (int)OvenRowCol.MaxCol; idx++)
            {
                ...
                if (PalletStatus.NG == this.dryingOvenRun.Pallet[ptlId].State)
                {
                    // 提示判断
                    if (DialogResult.Yes == MessageBox.Show("检测到此炉腔存在NG夹具，确认是否电池转盘?", "提示", MessageBoxButtons.YesNo))
                    {
                        this.UnBindPallet = true; // 初始化=false
                        break;
                    }
                }
            }
        ...
        private void RunWhile()
        {
            ...
            switch(this.dryOvenCmd)
            {
                ...
                case DryOvenCmd.DoorOpenClose:              // 炉门打开/关闭（写入）
                    {
                        ...
                        if(MachineCtrl.GetInstance().SafeDoorStateOpen)
                        {
                            msg = "安全门打开时不能操作干燥炉炉门";
                            break;
                        }
                ...
                case DryOvenCmd.FaultReset:                 // 故障复位（写入）
                    {
                        ...
                        result = this.dryingOvenRun.DryOvenFaultReset(cavityIdx, cavityData, true, OptMode.Manual, UnBindPallet);
                        //Thread.Sleep(500);
                ...
                case DryOvenCmd.AlarmReset:                 // 报警复位（写入）
                    {
                        cavityData.alarmState = (short)this.dryOvenCmdState;
                        result = this.dryingOvenRun.DryOvenAlarmReset(cavityIdx, cavityData, true, OptMode.Manual);
                        //Thread.Sleep(500);
                        //cavityData.faultReset = (short)OvenStatus.FaultResetOff;
                        //result = this.dryingOvenRun.DryOvenFaultReset(cavityIdx, cavityData, true, OptMode.Manual);
                        //msg = ("故障复位") + (result ? "成功" : "失败");
                        msg = result ? "解除报警成功" : "非报警状态，无法报警维修";
                        break;
                    }
                default:
                    msg = "操作指令错误";
        ...
        private void UpdataDryingOvenData()
        {
            ...
            idx = 0;
            this.listViewParameter.Items[idx++].SubItems[1].Text = "";//通用参数设定
            ...
            this.listViewParameter.Items[idx++].SubItems[1].Text = data.parameter.OpenDoorVacPressure.ToString(); //开门真空压力
            this.listViewParameter.Items[idx++].SubItems[1].Text = data.parameter.OpenDoorBlowTime.ToString(); //充氮气报警时间

            //this.listViewParameter.Items[idx++].SubItems[1].Text = data.parameter.BStateBlowAirTime.ToString(); //呼吸充干燥气时间
            ...
            this.listViewParameter.Items[idx++].SubItems[1].Text = ""; //换气阶段
            ...
            this.listViewParameter.Items[idx++].SubItems[1].Text = ""; //预热阶段
            ...
            this.listViewParameter.Items[idx++].SubItems[1].Text = ""; //真空呼吸阶段
            this.listViewParameter.Items[idx++].SubItems[1].Text = data.parameter.VacHeatTime.ToString(); //真空加热总时间
            this.listViewParameter.Items[idx++].SubItems[1].Text = data.parameter.AStateVacMaxValue.ToString(); //A状态真空最大值
            this.listViewParameter.Items[idx++].SubItems[1].Text = data.parameter.AStateSavePressureTime.ToString(); //A状态保压时间
            this.listViewParameter.Items[idx++].SubItems[1].Text = data.parameter.BStateVacTime.ToString(); //B状态抽真空报警时间
            this.listViewParameter.Items[idx++].SubItems[1].Text = data.parameter.BStateVacPressure.ToString();  //B状态真空压力
            this.listViewParameter.Items[idx++].SubItems[1].Text = data.parameter.BStateVacMaxValue.ToString(); //B状态真空最大值
            this.listViewParameter.Items[idx++].SubItems[1].Text = data.parameter.BStateSavePressureTime.ToString(); //B状态保压时间

            this.listViewParameter.Items[idx++].SubItems[1].Text = data.parameter.BStateBlowAirKeepTime.ToString(); //呼吸充干燥气保持时间
            this.listViewParameter.Items[idx++].SubItems[1].Text = data.parameter.BStateBlowAirPressure.ToString(); //呼吸充干燥气压力
            this.listViewParameter.Items[idx++].SubItems[1].Text = data.parameter.BreathCycleTimes.ToString();//呼吸循环次数
            ...
            //this.listViewParameter.Items[idx++].SubItems[1].Text = data.parameter.TempDifferAlarmValue.ToString();
        ...
        private void btnAlarmReset_Click(object sender, EventArgs e)
        {
            if (DryOvenCmd.End == this.dryOvenCmd)
            {
                this.dryOvenCmdState = OvenStatus.AlarmResetOn;
                this.dryOvenCmd = DryOvenCmd.AlarmReset;
            }
            else
            {
                ShowMsgBox.ShowDialog("干燥炉动作中，请稍后再操作...", MessageType.MsgMessage);
