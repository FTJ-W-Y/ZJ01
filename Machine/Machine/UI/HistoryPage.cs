...
namespace Machine
{
    public partial class HistoryPage : FormEx
    {
        ...
        private void CreateListView()
        {
            ...
            this.comboBoxType.Items.Add("夹具烘烤记录");

            this.dateTimePickerStart.CustomFormat = Def.DateFormal;
            this.dateTimePickerEnd.CustomFormat = Def.DateFormal;

            this.dateTimePickerStart.Value = DateTime.Parse( DateTime.Now.ToString("yyyy-MM-dd") );
            this.dateTimePickerEnd.Value = DateTime.Parse(DateTime.Now.ToString("yyyy-MM-dd 23:59:59"));
        ...
        private void buttonQuery_Click(object sender, EventArgs e)
        {
            ...
            switch(queryType)
            {
                case QueryType.Alarm:
                    {
                        ...
                        {
                            this.dataTable = this.dataTable.Rows.Cast<DataRow>().OrderByDescending(r => r[AlarmTable[(int)RecordColumn.ALM_ALARM_TIME]]).CopyToDataTable();
                        }
                ...
                case QueryType.DryingOvenLog:
                    {
                        ...
                        if(this.dataTable.Rows.Count > 0)
                        {
                            this.dataTable = this.dataTable.Rows.Cast<DataRow>().OrderByDescending(r => r[DryingOvenLogColumn.OptDate.ToString()]).CopyToDataTable();
                        }
                ...
                case QueryType.ParameterLog:
                    {
                        ...
                        if(this.dataTable.Rows.Count > 0)
                        {
                            this.dataTable = this.dataTable.Rows.Cast<DataRow>().OrderByDescending(r => r[ParameterLogColumn.OptDate.ToString()]).CopyToDataTable();
                        }
                ...
                case QueryType.RobotLog:
                    {
                        ...
                        if(this.dataTable.Rows.Count > 0)
                        {
                            this.dataTable = this.dataTable.Rows.Cast<DataRow>().OrderByDescending(r => r[RobotLogColumn.OptDate.ToString()]).CopyToDataTable();
                        }
                ...
                case QueryType.PalletLog:
                    {
                        modIdx--;
                        if (!DataBaseLog.GetPalletList(Def.GetProductFormula(), modIdx, startTime, endTime, ref this.dataTable))
                        {
                            return;
                        }
                        if (this.dataTable.Rows.Count > 0)
                        ...
                        this.dataTable.Columns[(int)MotorLogColumn.FormulaID].ColumnName = "夹具编号";
                        this.dataTable.Columns[(int)MotorLogColumn.Operater].ColumnName = "烘烤次数";
                        this.dataTable.Columns[(int)MotorLogColumn.OptDate].ColumnName = "烘烤时间";
                        this.dataTable.Columns[(int)MotorLogColumn.MotorID].ColumnName = "烘烤炉层";
                        this.dataTable.Columns[(int)MotorLogColumn.MotorName].ColumnName = "保养时间";
