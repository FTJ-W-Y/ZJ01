...
namespace Machine
{
    public partial class MaintenancePage : FormEx
    {
        ...
        private void CreateModuleListView()
        {
            ...
            {
                if (MachineCtrl.GetInstance().ListRuns[i].RunName.Equals("系统")|| MachineCtrl.GetInstance().ListRuns[i].RunName.Equals("调度机器人"))
                {
        ...
        {
            if (this.dataGridViewModule.CurrentCell.RowIndex == 1)
            {
                ModuleManager.GetInstance().SetCurModule(9);
            }
            else
            {
                // 保存选择的模组索引
        ...
        private void UpdateModuleIOStateMotorState(object sender, System.Timers.ElapsedEventArgs e)
        {
                    ...
                    for(int i = 1; i < num + 1; i++)
                    {
                        ...
                        if(null != ioBtn)
                        {
                            ...
                            if(this.lstInput.Count > ioIdx)
                            {
                                ...
                                //if (this.lstInput[ioIdx].IsOn())
                                //{
                                //    Trace.WriteLine(ioBtn.Text + $"信号{this.lstInput[ioIdx].IsOn()}");
                                //}
            ...
            {
                Def.WriteLog("MaintenancePage UpdateModuleIOStateMotorState()", $" error: {ex.Message}\r\n{ex.StackTrace}");
            }
        }
        ...
        private void OutputBtn_Click(object sender, EventArgs e)
        {
            ...
            if (null != ioBtn)
            {
                ...
                if (null != output)
                {
                    bool ison = !output.IsOn();
                    this.runCtrl.ManualDebugThread.IssueOutput(output, ison);
                }
            }
        }
        #endregion
        ...
        private bool CheckOperateSafe()
        {
            ...
            if(MachineCtrl.GetInstance().SafeDoorStateOpen && (MachineCtrl.GetInstance().dbRecord.UserLevel() > UserLevelType.USER_MAINTENANCE))
            {
