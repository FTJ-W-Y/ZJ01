...
namespace Machine
{
    public partial class MesInfoPage : FormEx
    {
        ...
        private void CreateListViewModule()
        {
            ...
            txtBoxCol = new DataGridViewTextBoxColumn();
            txtBoxCol.HeaderText = "资源编码";
            idx = dgv.Columns.Add(txtBoxCol);
            txtBoxCol = new DataGridViewTextBoxColumn();
            txtBoxCol.HeaderText = "工序编号";
            idx = dgv.Columns.Add(txtBoxCol);
            txtBoxCol = new DataGridViewTextBoxColumn();
            txtBoxCol.HeaderText = "MesURL";
            idx = dgv.Columns.Add(txtBoxCol);
            txtBoxCol = new DataGridViewTextBoxColumn();
            txtBoxCol.HeaderText = "操作员账号";
            idx = dgv.Columns.Add(txtBoxCol);
            txtBoxCol = new DataGridViewTextBoxColumn();
            txtBoxCol.HeaderText = "操作员密码";
            idx = dgv.Columns.Add(txtBoxCol);
            ...
            // 添加需要配置的资源ID
            int colIdx, rowIdx;
            string[] strRes = new string[] { "Equipment" };
            ResourcesStruct[] rs = new ResourcesStruct[] { MesResources.Equipment };
            for(int i = 0; i < rs.Length; i++)
            {
                ...
                dgv.Rows[rowIdx].Cells[colIdx++].Value = rs[i].EquipmentCode;
                dgv.Rows[rowIdx].Cells[colIdx++].Value = rs[i].ResourceCode;
                dgv.Rows[rowIdx].Cells[colIdx++].Value = rs[i].ProcessCode;
                dgv.Rows[rowIdx].Cells[colIdx++].Value = rs[i].MesURL;
                dgv.Rows[rowIdx].Cells[colIdx++].Value = rs[i].OperatorUserID;
                dgv.Rows[rowIdx].Cells[colIdx++].Value = rs[i].OperatorPassword;
            }
            this.textBoxHeartbeat.Text = MesResources.HeartbeatInterval.ToString();
            this.textBoxStopUpdata.Text = MesResources.StopUpdataInterval.ToString();
        ...
        private void buttonSave_Click(object sender, EventArgs e)
        {
            ...
            for(int i = 0; i < dgv.RowCount; i++)
            {
                ...
                int idx = 0;
                idx++;
                MesResources.Equipment.EquipmentCode = (null != dgv.Rows[i].Cells[idx].Value) ? dgv.Rows[i].Cells[idx].Value.ToString() : "";
                idx++;
                MesResources.Equipment.ResourceCode = (null != dgv.Rows[i].Cells[idx].Value) ? dgv.Rows[i].Cells[idx].Value.ToString() : "";
                idx++;
                MesResources.Equipment.ProcessCode = (null != dgv.Rows[i].Cells[idx].Value) ? dgv.Rows[i].Cells[idx].Value.ToString() : "";
                idx++;
                MesResources.Equipment.MesURL = (null != dgv.Rows[i].Cells[idx].Value) ? dgv.Rows[i].Cells[idx].Value.ToString() : "";
                idx++;
                MesResources.Equipment.OperatorUserID = (null != dgv.Rows[i].Cells[idx].Value) ? dgv.Rows[i].Cells[idx].Value.ToString() : "";
                idx++;
                MesResources.Equipment.OperatorPassword = (null != dgv.Rows[i].Cells[idx].Value) ? dgv.Rows[i].Cells[idx].Value.ToString() : "";
            }
            int.TryParse(this.textBoxHeartbeat.Text, out MesResources.HeartbeatInterval);
            int.TryParse(this.textBoxStopUpdata.Text, out MesResources.StopUpdataInterval);
                    ...
                    case UIEnable.AllEnabled:
                        {
                            ...
                            this.textBoxStopUpdata.Enabled = en;
