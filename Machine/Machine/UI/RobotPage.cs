...
namespace Machine
{
    public partial class RobotPage : FormEx
    {
        ...
        public bool robotRunning;              // 机器人运行中
        ...
        public override void UpdataUIEnable(SystemControlLibrary.MCState mc, SystemControlLibrary.UserLevelType level)
        {
            try
            {
                if (level == UserLevelType.USER_LOGOUT)
                {
                    this.Enabled = false;
                    return;
                }
                ...
                else
                {
                    ...
                    if(this.comboBoxTransferRobot.Items.Count > 0)
                    {
                        ...
                        if(null != run)
                        {
                            ...
                            // 感应器装配支架有问题取消屏蔽按钮，改支架后需要删除，2022.0303-Anbin
                            else if (true)
                            {
        ...
        private void OnloadRobotMove(RobotOrder order)
        {
            ...
            {
                ShowMsgBox.ShowDialog("机器人动作中，请稍后再操作...", MessageType.MsgMessage);
            }
        }
        ...
        private void TransferRobotMove(RobotOrder order)
        {
            ...
            {
                ShowMsgBox.ShowDialog(string.Format("机器人动作异常:{0}",ex.ToString()), MessageType.MsgAlarm);
            }
        }
