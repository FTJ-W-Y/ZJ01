                ...
                case AutoSteps.Auto_WaitWorkStart:
                    {
                        ...
                        //OutputAction(ORequireSendEnd, InputState(IResponseSend, true));  // 接收完电池
                        ...
                        {
                            if (Def.IsNoHardware() || CheckInputState(IRecvPosInpos, true))
                            {
                        ...
                        {
                            // 
                            if (InputState(IResponseSend, false))
                            {
                                OutputAction(ORequireSendEnd, false);
                            }
                        }
                        break;
                    }
                ...
                case AutoSteps.Auto_RecvBatttery:
                    {
                        ...
                        if (PositionPush(true))
                        {
                            ...
                            OutputAction(ORequireSendEnd, false);  // 接收完电池
                            OutputAction(ORequireSend, this.conveyerLineEN);    // 可以接收电池

                            bool recvFin = false;
                            bool recvEnter = false;
                            DateTime time = DateTime.Now;
                            while (true)
                            {
                                // 接收位信号
                                if (!recvEnter && InputState(IRecvPosEnter, true))
                                {
                                    recvEnter = true;// 接收完电池
                                }
                                if (recvEnter) //接收完成，入料信号取消之前一直给完成信号
                                {
                                    OutputAction(ORequireSend, false);      // 取消入料信号
                                    OutputAction(ORequireSendEnd, InputState(IResponseSend, true));   // 与上工序交互完成
                                }
                                if (InputState(IRecvPosInpos, true) && InputState(IRecvPosEnter, false))
                                {
                                    Sleep(recvDelay); //延迟等待扫码
                                    recvFin = true;
                                    break;
                                }
                                if(this.DryRun)
                                ...
                                if((int)(DateTime.Now - time).TotalSeconds > recvTimeOut)
                                {
                                    recvFin = false;
                                    break;
                                }
                            ...
                            OutputAction(ORequireSend, false);
                            OutputAction(ORequireSendEnd, false);
                            if (recvFin)
                            {
                ...
                case AutoSteps.Auto_MesCheckCode:
                    {
                        ...
                        for(int i = 0; i < this.Battery.Length; i++)
                        {
                            ...
                            {
                                //this.Battery[i].Code = $"09UCE11E000002BBQ226000{i + 11}";
                                string msg = "";
                                if (!Def.IsNoHardware() && MachineCtrl.GetInstance().UpdataMes && !MesOperate.EquToMesCheckSfc(MesResources.Equipment, this.Battery[i].Code, ref msg))
                                {
