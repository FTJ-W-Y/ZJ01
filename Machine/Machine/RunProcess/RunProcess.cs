...
namespace Machine
{
    ...
    public class RunProcess : RunEx
    {
        ...
        protected Dictionary<string, ParameterFormula> insertParameterList;      // 模组中插入的参数集：<参数关键字key, 参数样式>
        ...
        public virtual void ManulSetAutoStep(int step)
        {
        ...
        /// </summary>
        /// <returns></returns>
        public virtual bool MotorMoveToSafePos()
        {
            return true;
        }
        ...
        public virtual void LoadRunData()
        {
            ...
            for (int i = 0; i < this.Pallet.Length; i++)
            {
                ...
                // 处理切换夹具类型电池行不一样 wjj 220918
                if (maxRow > this.Pallet[i].Battery.GetLength(0))
                {
                    maxRow = this.Pallet[i].Battery.GetLength(0);
                }
                if (maxCol > this.Pallet[i].Battery.GetLength(1))
                {
                    maxCol = this.Pallet[i].Battery.GetLength(1);
                }
                // 处理切换夹具类型电池行不一样 wjj 220918
                ...
                key = $"Pallet[{i}].OutBak";
                this.Pallet[i].OutBak = iniStream.ReadBool(section, key, this.Pallet[i].OutBak);
                ...
                key = $"Pallet[{i}].LogonCheckDate";
                DateTime.TryParse(iniStream.ReadString(section, key, this.Pallet[i].LogonCheckDate.ToString(Def.DateFormal)), out this.Pallet[i].LogonCheckDate);
            ...
            if (SaveType.Pallet == (SaveType.Pallet & saveType))
            {
                for (int i = 0; i < this.Pallet.Length; i++)
                {
                    if ((i == index) || (index < 0))
                    {
                        ...
                        key = $"Pallet[{i}].OutBak";
                        iniStream.WriteBool(section, key, this.Pallet[i].OutBak);
                        key = $"Pallet[{i}].BakingCount";
                        ...
                        key = $"Pallet[{i}].LogonCheckDate";
                        iniStream.WriteString(section, key, this.Pallet[i].LogonCheckDate.ToString(Def.DateFormal));
                        key = $"Pallet[{i}].StartDate";
        ...
        {
            int value = IniFile.ReadInt(this.RunModule, key, defaultValue, Def.GetAbsPathName(Def.ModuleExCfg));
        ...
        protected int ReadIntParameter(string section, string key, int defaultValue)
        {
            try
            {
        ...
        protected string ReadStringParameter(string section, string key, string defaultValue)
        {
            try
            {
                if ((section == this.RunModule) && this.dataBaseParameterList.ContainsKey(key))
        ...
        protected override void InsertAlarmInfo(int msgID, string msg, int msgType, int runModuleID, string runName, int productFormula, string curTime)
        {
            try
            {
                ...
                {
                    //MesOperate.EquToMesAlarm(MesResources.Equipment, MesAlarmStatus.Happen, $"{msgID}", $"{runName} {msg}", ref msg);
                    //MesOperate.EquToMesAlarm(MesResources.Equipment, MesAlarmStatus.Happen, $"{msgID}", $"{runName} {msg}", MesAlarmLevel.Zero.ToString(), ref msg);
                }
            }
            catch (System.Exception ex)
            {
        ...
        /// 模组报警，外部调用
        /// </summary>
        public void ModuleShowMessageID(int msgID, string[] addMsg = null, int countdownTime = 0)
        {
            ShowMessageID(msgID, addMsg, countdownTime, System.Windows.Forms.DialogResult.None);
        }
        ...
        /// <param name="batState">夹具最大列</param>
        /// <param name="pltState">夹具最大列</param>
        public virtual void ManualAddPalletBattery(int pltIdx, int maxRow, int maxCol, bool isFake, PalletStatus pltState, BatteryStatus batState)
        {
        ...
        public virtual void ManualDelBattery(int pltIdx, int rowIdx, int colIdx)
        {
        ...
        /// <param name="rowIdx"></param>
        /// <param name="colIdx"></param>
        public virtual void ManualAddBattery(int pltIdx, int rowIdx, int colIdx, string code)
        {
        ...
        /// <param name="rowIdx"></param>
        /// <param name="colIdx"></param>
        public virtual void ManualGetBattery(int pltIdx, int rowIdx, int colIdx, ref string code)
        {
        }
        ...
        /// <param name="batState">夹具最大列</param>
        /// <param name="pltState">夹具最大列</param>
        public virtual void ManualResetEvent()
        {
        ...
        public virtual void OutMESEvent(Pallet plt, int cavityIdx)
        {

        }
        #endregion
        //设置上下料夹具信息
        public virtual void SetPallet(int pltIdx, Pallet pallet, bool place)
        {

        }
        //设置夹具类型
        public virtual void SetBarCodeType(string code)
        {

        }
        //public virtual bool Checked(LoadingCmd loadingCmd,OnloadData onloadData) 
        //{
        //    return true;
        //}
    }
