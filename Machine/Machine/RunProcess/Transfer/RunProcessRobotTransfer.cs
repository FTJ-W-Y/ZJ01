...
namespace Machine
{
    ...
    class RunProcessRobotTransfer : RunProcess
    {
        ...
        protected new enum AutoSteps
        {
            ...
            Auto_PickDryOvenAvoidMove,
            Auto_PickDryOvenIn,
            Auto_PickDryOvenGetData,
            Auto_PickDryOvenOut,
            Auto_PickDryOvenCheckFinger,
            Auto_PickDryOvenCheckAvoid,
            ...
            Auto_PlaceDryOvenAvoidMove,
            Auto_PlaceDryOvenBindMESData,
            Auto_PlaceDryOvenInMESData,
            Auto_PlaceDryOvenIn,
            Auto_PlaceDryOvenSetData,
            Auto_PlaceDryOvenOut,
            Auto_PlaceDryOvenCheckFinger,
            Auto_PlaceDryOvenCheckAvoid,
        ...
        /// 夹具放假电池夹具数
        /// </summary>
        public int placeFakePalletCnt;

        //调度优先级
        public int transferRobotPriority;
        ...
        private PickPlacePos pickOrPlaceAvoidPos;                      // 调度机器人避让位置
        ...
        public RunProcessRobotTransfer(int runId) : base(runId)
        {
            ...
            InsertVoidParameter("placeFakePalletCnt", "炉腔假电池夹具个数", $"炉腔夹具假电池个数：1=放一个假电池夹具；2=放两个假电池夹具", placeFakePalletCnt, RecordType.RECORD_INT, ParameterLevel.PL_STOP_MAIN);
            InsertVoidParameter("transferRobotPriority", "调度优先级", "调度优先级：0为水含量电池优先，1为上料优先，2为下料优先", transferRobotPriority, RecordType.RECORD_INT, ParameterLevel.PL_STOP_MAIN);
        }
        ...
        protected override void AutoOperation()
        {
            ...
            switch ((AutoSteps)this.nextAutoStep)
            {
                ...
                case AutoSteps.Auto_WaitWorkStart:
                    {
                        ...
                        if (transferRobotPriority==0)
                        {
                            ...
                            // 下料料待测假电池，先保证电池上下料
                            else if (CalcOnloadPlaceDetectFake(ref this.pickPos, ref this.placePos))
                            {
                                result = true;
                            }
                            ...
                            else if (CalcBufferPick(ref this.pickPos, ref this.placePos))
                            {
                                result = true;
                            }
                        }
                        else if (transferRobotPriority == 1)
                        {
                            //腔体转移
                            if (CalcTransferDryOven(ref this.pickPos, ref this.placePos))
                            {
                        ...
                        else if (transferRobotPriority == 2)
                        {
                            //腔体转移
                            if (CalcTransferDryOven(ref this.pickPos, ref this.placePos))
                            {
                ...
                case AutoSteps.Auto_CalcPickPalletPos:
                    {
                        ...
                        switch (this.pickPos.stationEvent)
                        {
                            ...
                            case EventList.PalletBufferPickOKFullPallet:
                            case EventList.PalletBufferPickOKFakeFullPallet:
                                {
                ...
                case AutoSteps.Auto_PickDryOvenSetEvent:
                    {
                        ...
                        //if (SetModuleEvent(pickPos.runID, pickPos.stationEvent, EventStatus.Response, pickPos.col))
                        {
                            if ((int)this.pickPos.runID == 24)
                            {
                                this.nextAutoStep = AutoSteps.Auto_PickDryOvenAvoidMove;
                            }
                            else
                            {
                ...
                case AutoSteps.Auto_PickDryOvenAvoidMove:
                    {
                        this.msgChs = string.Format("机器人移动到[{0}-{1}-{2}]", GetRobotStationName(this.pickPos.station), this.pickPos.row, this.pickPos.col);
                        this.msgEng = string.Format("Robot move to [{0}-{1}-{2}]", this.pickPos.station, this.pickPos.row, this.pickPos.col);
                        CurMsgStr(this.msgChs, this.msgEng);
                        this.pickOrPlaceAvoidPos.runID = this.pickPos.runID + 1;
                        this.pickOrPlaceAvoidPos.station = this.pickPos.station + 1;
                        this.pickOrPlaceAvoidPos.row = this.pickPos.row;
                        this.pickOrPlaceAvoidPos.col = this.pickPos.col;
                        this.pickOrPlaceAvoidPos.stationEvent = this.pickPos.stationEvent;
                        if (GetRobotCmd(pickOrPlaceAvoidPos.station, pickOrPlaceAvoidPos.row, pickOrPlaceAvoidPos.col, robotSpeed, RobotOrder.MOVE, ref robotCmd))
                        {
                            if (RobotMove(robotCmd, true))
                            {
                                RunProcessOnloadRobot runOnload = MachineCtrl.GetInstance().GetModule(RunID.OnloadRobot) as RunProcessOnloadRobot;
                                runOnload.transAvoid = 0;

                                if (runOnload.onloadData.avoidMove)
                                {
                                    this.nextAutoStep = AutoSteps.Auto_PickDryOvenMove;
                                }
                            }
                ...
                case AutoSteps.Auto_PickDryOvenGetData:
                    {
                        ...
                        {
                            this.Pallet[0].Copy(tmpPlt);
                            //if (tmpPlt.State== PalletStatus.NG)
                            //{
                            //    string dryingOvenCode = "WH02C0122PR-HKX002" + (11 + Convert.ToInt32(MachineCtrl.GetInstance().GetModule(pickPos.runID).RunName.Replace("干燥炉", ""))); //设备编号
                            //    MachineCtrl.GetInstance().CANCELINBOUND_Main(MesResources.Equipment, this.Pallet[0], dryingOvenCode, ref msg);
                            //}
                ...
                case AutoSteps.Auto_PickDryOvenOut:
                    {
                        // 这里正在取出时，出现异常，需手动设置动作完成，否则自动模式下可能会跟初始化关炉门冲突
                ...
                case AutoSteps.Auto_PickDryOvenCheckFinger:
                    {
                        ...
                        this.msgEng = string.Format("Check [{0}-{1}-{2}] station pallet", this.pickPos.station, this.pickPos.row, this.pickPos.col);
                        CurMsgStr(this.msgChs, this.msgEng);
                        if ((this.Pallet[0].State == PalletStatus.WaitOffload && this.Pallet[0].Stage == PalletStage.Baked && !this.Pallet[0].IsEmpty())|| (this.Pallet[0].State == PalletStatus.NG && !this.Pallet[0].IsEmpty()))
                        {
                            string dryingOvenCode = "WH02C0122PR-HKX002" + (11 + Convert.ToInt32(MachineCtrl.GetInstance().GetModule(pickPos.runID).RunName.Replace("干燥炉", ""))); //设备编号

                            ////MES 托盘电池解绑
                            //if (!MachineCtrl.GetInstance().ACINBOUND_Main(MesResources.Equipment, this.Pallet[0], dryingOvenCode, this.RunName, true, false, ref msg))
                            //{
                        ...
                        if (PalletKeepFlat(0, true))
                        {
                            ...
                            if (SetModuleEvent(pickPos.runID, pickPos.stationEvent, EventStatus.Finished, pltIdx))
                            {
                                if ((int)this.pickPos.runID == 24)
                                {
                                    this.nextAutoStep = AutoSteps.Auto_PickDryOvenCheckAvoid;
                                    SaveRunData(SaveType.AutoStep);
                                }
                                else
                                {
                ...
                case AutoSteps.Auto_PickDryOvenCheckAvoid:
                    {
                        this.msgChs = string.Format("机器人在[{0}-{1}-{2}]取料后检查安全位", GetRobotStationName(this.pickPos.station), this.pickPos.row, this.pickPos.col);
                        this.msgEng = string.Format("Check [{0}-{1}-{2}] station pallet", this.pickPos.station, this.pickPos.row, this.pickPos.col);
                        CurMsgStr(this.msgChs, this.msgEng);
                        this.pickOrPlaceAvoidPos.runID = this.pickPos.runID + 1;
                        this.pickOrPlaceAvoidPos.station = this.pickPos.station + 1;
                ...
                case AutoSteps.Auto_PickOffloadGetData:
                    {
                        ...
                        if (GetModulePallet(pickPos.runID, pickPos.col, ref tmpPlt))
                        {
                            ...
                            {
                                tmpPlt.OutBak = false;
                                tmpPlt.BakingCount = 0;
                                tmpPlt.LogonCheckDate = DateTime.MinValue;
                                tmpPlt.StartDate = DateTime.MinValue;
                                tmpPlt.EndDate = DateTime.MinValue;
                            }
                            this.Pallet[0].Copy(tmpPlt);
                            tmpPlt.Release();
                            if (SetModulePallet(pickPos.runID, pickPos.col, tmpPlt))
                ...
                case AutoSteps.Auto_CalcPlacePalletPos:
                    {
                        ...
                        switch (this.placePos.stationEvent)
                        {
                            ...
                            case EventList.PalletBufferPlaceOKFullPallet:
                            case EventList.PalletBufferPlaceOKFakeFullPallet:
                                {
                            ...
                            //case EventList.OffLoadPlaceDetectFakePallet:  //下料区放待检测含假电池夹具（未取走假电池的夹具）
                ...
                case AutoSteps.Auto_PlaceOnloadSetData:
                    {
                        ...
                        if (SetModulePallet(placePos.runID, placePos.col, this.Pallet[0], true))
                        {
                            ////判断夹具状态是否为OK状态
                            //if (this.Pallet[0].State == PalletStatus.NG && !this.Pallet[0].IsEmpty())
                            //{
                            ...
                            //    //string dryingOvenCode = "WH02C0122PR-HKX002" + (11 + Convert.ToInt32(this.RunName.Replace("干燥炉", ""))); //设备编号
                            //    //MES 托盘电池解绑
                            //    if (!MachineCtrl.GetInstance().ACINBOUND_Main(MesResources.Equipment, this.Pallet[0], "WH02C0122PR-HKX00211", "烘烤调度线体", true, false, ref msg))
                            //    {
                            //        ShowMsgBox.ShowDialog($"{msg}", MessageType.MsgAlarm);
                            //        break;
                            //    }
                ...
                case AutoSteps.Auto_PlaceBufferSetEvent:
                    {
                        ...
                        //if (SetModuleEvent(placePos.runID, placePos.stationEvent, EventStatus.Response, placePos.col))
                        // wjj 220921
                        if (SetModuleEvent(placePos.runID, placePos.stationEvent, EventStatus.Response, placePos.row))
                        {
                ...
                case AutoSteps.Auto_PlaceDryOvenSetEvent:
                    {
                        ...
                        //if(SetModuleEvent(placePos.runID, placePos.stationEvent, EventStatus.Response, placePos.col))
                        {
                            if ((int)this.placePos.runID == 24)
                            {
                                this.nextAutoStep = AutoSteps.Auto_PlaceDryOvenAvoidMove;
                            }
                            else
                            {
                ...
                case AutoSteps.Auto_PlaceDryOvenAvoidMove:
                    {
                        this.msgChs = string.Format("机器人移动到[{0}-{1}-{2}]", GetRobotStationName(this.placePos.station), this.placePos.row, this.placePos.col);
                        this.msgEng = string.Format("Robot move to [{0}-{1}-{2}]", this.placePos.station, this.placePos.row, this.placePos.col);
                        CurMsgStr(this.msgChs, this.msgEng);
                        this.pickOrPlaceAvoidPos.runID = this.placePos.runID + 1;
                        this.pickOrPlaceAvoidPos.station = this.placePos.station + 1;
                        this.pickOrPlaceAvoidPos.row = this.placePos.row;
                        this.pickOrPlaceAvoidPos.col = this.placePos.col;
                        this.pickOrPlaceAvoidPos.stationEvent = this.placePos.stationEvent;
                        if (GetRobotCmd(pickOrPlaceAvoidPos.station, pickOrPlaceAvoidPos.row, pickOrPlaceAvoidPos.col, robotSpeed, RobotOrder.MOVE, ref robotCmd))
                        {
                            if (RobotMove(robotCmd, true))
                            {
                ...
                //case AutoSteps.Auto_PlaceDryOvenBindMESData:
                //    {
                //        this.msgChs = string.Format("进行MES托盘绑定");
                //        this.msgEng = string.Format("BindPalletMES");
                //        CurMsgStr(this.msgChs, this.msgEng);
                //        if ((this.Pallet[0].State == PalletStatus.OK || this.Pallet[0].State == PalletStatus.Turn) && this.Pallet[0].Stage == PalletStage.Onload && !this.Pallet[0].IsEmpty())
                //        {
                //            string dryingOvenCode = "WH02C0122PR-HKX002" + (11 + Convert.ToInt32(MachineCtrl.GetInstance().GetModule(placePos.runID).RunName.Replace("干燥炉", ""))); //设备编号
                //                                                                                                                                                                   //MES 托盘电池绑定
                //            if (!MachineCtrl.GetInstance().ACINBOUND_Main(MesResources.Equipment, this.Pallet[0], dryingOvenCode, this.RunName, true, true, ref msg))
                //            {
                //                ShowMsgBox.ShowDialog($"{msg}", MessageType.MsgAlarm);
                //                break;
                //            }
                //            this.nextAutoStep = AutoSteps.Auto_PlaceDryOvenInMESData;
                //            SaveRunData(SaveType.AutoStep);
                //        }
                //        break;
                //    }
                //case AutoSteps.Auto_PlaceDryOvenInMESData:
                //    this.msgChs = string.Format("进行MES托盘入站");
                //    this.msgEng = string.Format("InPalletMES");
                //    CurMsgStr(this.msgChs, this.msgEng);
                //    { //判断夹具状态是否为OK状态
                //        if ((this.Pallet[0].State == PalletStatus.OK || this.Pallet[0].State == PalletStatus.Turn) && this.Pallet[0].Stage == PalletStage.Onload && !this.Pallet[0].IsEmpty())
                //        {
                //            string dryingOvenCode = "WH02C0122PR-HKX002" + (11 + Convert.ToInt32(MachineCtrl.GetInstance().GetModule(placePos.runID).RunName.Replace("干燥炉", ""))); //设备编号
                //                                                                                                                                                                   //MES 托盘电池绑定
                //            if (this.Pallet[0].State == PalletStatus.OK && this.Pallet[0].Stage == PalletStage.Onload && !this.Pallet[0].IsEmpty())
                //            {
                //                if (!MachineCtrl.GetInstance().ACLOGONCHECK_Main(MesResources.Equipment, this.Pallet[0], dryingOvenCode, MachineCtrl.GetInstance().GetModule(placePos.runID).RunName, ref msg))
                //                {
                //                    ShowMsgBox.ShowDialog($"{msg}", MessageType.MsgAlarm);
                //                    break;
                //                }
                //            }
                //            this.Pallet[0].State = PalletStatus.OK;
                //            this.Pallet[0].LogonCheckDate = DateTime.Now;
                ...
                case AutoSteps.Auto_PlaceDryOvenCheckFinger:
                    {
                        ...
                        if (PalletKeepFlat(0, false))
                        {
                            ...
                            {
                                if ((int)this.placePos.runID == 25)
                                {
                                    this.nextAutoStep = AutoSteps.Auto_PlaceDryOvenCheckAvoid;
                                    SaveRunData(SaveType.AutoStep);
                                }
                                else
                                {
                ...
                case AutoSteps.Auto_PlaceDryOvenCheckAvoid:
                    {
                        this.msgChs = string.Format("机器人在[{0}-{1}-{2}]取料后检查安全位", GetRobotStationName(this.placePos.station), this.placePos.row, this.placePos.col);
                        this.msgEng = string.Format("Check [{0}-{1}-{2}] station pallet", this.placePos.station, this.placePos.row, this.placePos.col);
                        CurMsgStr(this.msgChs, this.msgEng);
                        this.pickOrPlaceAvoidPos.runID = this.placePos.runID + 1;
                        this.pickOrPlaceAvoidPos.station = this.placePos.station + 1;
                ...
                case AutoSteps.Auto_PlaceOffloadCheckFinger:
                    {
                        ...
                        if (PalletKeepFlat(0, false))
                        {
                            if (SetModuleEvent(placePos.runID, placePos.stationEvent, EventStatus.Finished, placePos.col))
                            {
                                ...
                                // 当前是否存在空夹具需插走 wjj 220912
                                if (CalcOffloadPick(ref this.pickPos, ref this.placePos))
                                {
                                    this.nextAutoStep = AutoSteps.Auto_PrsetPickEvent;
                                    SaveRunData(SaveType.AutoStep | SaveType.Variables);
                                }
        ...
        private void ResetEvent()
        {
            ...
            //查看关联模组
            for (TransferRobotStation station = TransferRobotStation.OnloadStation; station < TransferRobotStation.StationEnd; station++)
            {
                if (station >= TransferRobotStation.DryOven_0 || station <= TransferRobotStation.DryOven_All)
                {
                    runProcess = MachineCtrl.GetInstance().GetModule(RunID.DryOven0 + (station - TransferRobotStation.DryOven_0));
                    for (EventList i = EventList.DryOvenPlaceEmptyPallet; i < EventList.DryOvenPickPlaceEnd; i++)
                    {
                        state = GetEvent(runProcess, i);
                        if (!(state == EventStatus.Require || state == EventStatus.Invalid || state == EventStatus.Finished))
                        {
                            SetEvent(runProcess, i, EventStatus.Invalid);
                        }
                    }
                }
                switch (station)
                {
                    case TransferRobotStation.OnloadStation:
                        runProcess = MachineCtrl.GetInstance().GetModule(RunID.OnloadRobot);
                    ...
                    case TransferRobotStation.PalletBuffer:
                        runProcess = MachineCtrl.GetInstance().GetModule(RunID.PalletBuffer);
                    ...
                    case TransferRobotStation.ManualOperate:
                        runProcess = MachineCtrl.GetInstance().GetModule(RunID.ManualOperate);
                        ...
                        break;
                    //case TransferRobotStation.DryOven_0:
                    //case TransferRobotStation.DryOven_1:
                    ...
                    //    runProcess = MachineCtrl.GetInstance().GetModule(RunID.DryOven0 + (station - TransferRobotStation.DryingOven_0));
                    //    for (EventList i = EventList.OvenPlaceEmptyPlt; i < EventList.OvenEventEnd; i++)
                    //    {
                    //        EventStatus state = EventStatus.Invalid;
                    //        if (GetEvent(runProcess, i, ref state))
                    //        {
                    //            if (!(state == EventStatus.Require || state == EventStatus.Invalid || state == EventStatus.Finished))
                    //            {
                    //                SetEvent(runProcess, i, EventStatus.Invalid);
                    ...
                    case TransferRobotStation.OffloadStation:
                        runProcess = MachineCtrl.GetInstance().GetModule(RunID.OffloadBattery);
        ...
        protected override void InitParameter()
        {
            ...
            this.placeFakePalletCnt = 1;
            this.transferRobotPriority = 0;
            this.robotAlarm = false;
            this.robotCommAlarm = false;
            base.InitParameter();
        }
        ...
        public override bool ReadParameter()
        {
            ...
            this.placeFakePalletCnt = ReadIntParameter(this.RunModule, "placeFakePalletCnt", this.placeFakePalletCnt);
            this.transferRobotPriority = ReadIntParameter(this.RunModule, "transferRobotPriority", this.transferRobotPriority);
            if (placeFakePalletCnt != 1 && placeFakePalletCnt != 2)
            {
                placeFakePalletCnt = 1;
            }
        ...
        public override bool PalletKeepFlat(int pltIdx, bool hasPlt, bool alarm = true)
        {
            ...
            {
                //if(IPalletKeepFlat[i] > -1 && !InputState(IPalletKeepFlat[i], hasPlt))
                if (IPalletKeepFlat[i] > -1 && !InputState(IPalletKeepFlat[i], !hasPlt)) //wjj 220826
                {
                    if (alarm)
                    {
                        //CheckInputState(IPalletKeepFlat[i], hasPlt);
                        CheckInputState(IPalletKeepFlat[i], !hasPlt); //wjj 220826
                    }
                    return false;
                }
            }
        ...
        public bool RobotManulAvoid(TransferRobotStation station, int row, int col, RobotOrder order)
        {
            ...
            {
                //  夹具缓存架 wjj 220917
                if (TransferRobotStation.PalletBuffer == station || TransferRobotStation.ManualOperate == station)
                {
                    ShowMsgBox.ShowDialog("手动操作取放操作台和缓存架，操作完成后必须手动添加相应点位夹具或删除夹具!", MessageType.MsgMessage);
                }
                else //  夹具缓存架 wjj 220917
                {
        ...
        private bool CheckStationSafe(PickPlacePos pos, RobotOrder order)
        {
                ...
                default:
                    {
                        if ((pos.station >= TransferRobotStation.DryOven_0) && (pos.station <= TransferRobotStation.DryOven_All))
                        {
                            ...
                            if ((int)OvenStatus.PalletHave == pltState && order == RobotOrder.PLACEIN)
                            {
                                msg = string.Format("{0}-{1}-{2}工位有夹具，不能实现放进操作"
                                    , GetRobotStationName(pos.station), (pos.row + 1), pos.col + 1);
                                ShowMessageBox((int)MsgID.OvenSafetyCurtainErrr, msg, "", MessageType.MsgAlarm);
                                return false;
                            }
                        }
            ...
            if (RobotOrder.PICKIN == order)
            {
                if (!PalletKeepFlat(0, false, false))
                {
                    ...
                    dispose = string.Format("请检查插料架夹具感应器正确后再启动软件", GetRobotStationName(pos.station));
        ...
        private bool CalcOnloadPick(ref PickPlacePos pick, ref PickPlacePos place)
        {
            ...
            if (SearchOnloadPickPos(EventList.OnloadPickOKFullPallet, ref pick))
            {
                ...
                // 缓存架放OK满料夹具
                if (SearchPalletBufferPlacePos(EventList.PalletBufferPlaceOKFullPallet, ref place))
                {
                    return true;
                }
            }
            ...
            if (SearchOnloadPickPos(EventList.OnloadPickOKFakeFullPallet, ref pick))
            {
                ...
                // 缓存架放OK满料带假电池夹具
                if (SearchPalletBufferPlacePos(EventList.PalletBufferPlaceOKFakeFullPallet, ref place))
                {
                    return true;
                }
            }
            ...
            if (SearchOnloadPickPos(EventList.OnLoadPickWaitResultPallet, ref pick))
            {
                    ...
                    && GetModulePallet(RunID.OnloadRobot, pick.col, ref pickPlt))
                {
                    if (((int)TransferRobotStation.DryOven_0 <= pickPlt.SrcStation) && (pickPlt.SrcStation <= (int)TransferRobotStation.DryOven_All))
                    {
                        ...
                        int pltIdx = -1;
                        bool PalletOK = GetModulePallet(ovenId, (pickPlt.SrcRow * (int)OvenRowCol.MaxCol + pickPlt.SrcCol), ref placePlt);
                        EventStatus ModuleStatus = GetModuleEvent(ovenId, EventList.DryOvenPlaceWaitResultPallet, ref pltIdx);
                        if ((EventStatus.Require == ModuleStatus)
            ...
            if (SearchOnloadPickPos(EventList.OnloadPickRebakeFakePallet, ref pick))
            {
                    ...
                    && GetModulePallet(RunID.OnloadRobot, pick.col, ref pickPlt))
                {
                    if (((int)TransferRobotStation.DryOven_0 <= pickPlt.SrcStation) && (pickPlt.SrcStation <= (int)TransferRobotStation.DryOven_All))
                    {
                        ...
                        int pltIdx = -1;
                        RunID ovenId = RunID.DryOven0 + (pickPlt.SrcStation - (int)TransferRobotStation.DryOven_0);
                        EventStatus status = GetModuleEvent(ovenId, EventList.DryOvenPlaceRebakeFakePallet, ref pltIdx);
                        if ((EventStatus.Require == status)
                            && (pltIdx > -1 && pltIdx < (int)ModuleMaxPallet.DryingOven)
                            && (pickPlt.SrcRow > -1 && pickPlt.SrcRow < (int)OvenRowCol.MaxRow)
                            && (pickPlt.SrcCol > -1 && pickPlt.SrcCol < (int)OvenRowCol.MaxCol)
                            && GetModuleEnable(ovenId) && GetModuleRunning(ovenId)
        ...
        private bool CalcOnloadPlaceDetectFake(ref PickPlacePos pick, ref PickPlacePos place)
        {
        ...
        private bool CalcOffloadPlace(ref PickPlacePos pick, ref PickPlacePos place)
        {
            ...
            // 上下料优先 // wjj 220830
            if (SearchOffloadPlacePos(EventList.OffLoadPlaceDryFinishPallet, ref place))
            {
                // 干燥炉取干燥完成夹具（等待下料）
        ...
        /// <param name="place"></param>
        /// <returns></returns>
        private bool CalcBufferPick(ref PickPlacePos pick, ref PickPlacePos place)
        {
            // 人工操作台取OK满料夹具
            if (SearchPalletBufferPickPos(EventList.PalletBufferPickOKFullPallet, ref pick))
            {
                // 干燥炉放Ok满料夹具
                if (SearchGlobalsDryOven(EventList.DryOvenPlaceOnlOKFullPallet, ref place, false, true))
                {
                    return true;
                }
            }
            // 人工操作台取OK满料带假电池夹具
            if (SearchPalletBufferPickPos(EventList.PalletBufferPickOKFakeFullPallet, ref pick))
            {
                // 干燥炉放Ok满料带假电池夹具
                if (SearchGlobalsDryOven(EventList.DryOvenPlaceOnlOKFakeFullPallet, ref place, false, true))
                {
                    return true;
                }
            }
        ...
        private bool CalcTransferDryOven(ref PickPlacePos pick, ref PickPlacePos place)
        {
            ...
            for (RunID pickId = RunID.DryOven0; pickId < RunID.DryOvenALL; pickId++)
            {
                ...
                EventStatus pickState = GetModuleEvent(pickId, EventList.DryOvenPickTransferPallet, ref pltPickIdx);
                if (EventStatus.Invalid == pickState || EventStatus.Finished == pickState)
                {
                    continue; // wjj 220915
                }

                // 这里进入转炉操作之后两边炉门打开一直等到转炉完成，中间炉子没有再触发请求
                int pickRow = pltPickIdx / (int)OvenRowCol.MaxCol;
                for (int pickCol = 0; pickCol < (int)OvenRowCol.MaxCol; pickCol++)
                {
                    ...
                    {
                        if (GetModulePallet(pickId, (pickRow * (int)OvenRowCol.MaxCol) + pickCol, ref plt) && (plt.State > PalletStatus.Invalid))
                        {
                            // 获取接收转移夹具
                            for (RunID placeId = RunID.DryOven0; placeId < RunID.DryOvenALL; placeId++)
                            {
                                ...
                                EventStatus placeState = GetModuleEvent(placeId, EventList.DryOvenPlaceTransferPallet, ref pltPlaceIdx);
                                if (pickState == placeState)
                                {
                                    int placeRow = pltPlaceIdx / (int)OvenRowCol.MaxCol;
                                    for (int placeCol = 0; placeCol < (int)OvenRowCol.MaxCol; placeCol++)
                                    {
                                        if (GetDryingOvenCavityTransferRecv(placeId, placeRow) && GetModulePallet(placeId, (placeRow * (int)OvenRowCol.MaxCol) + placeCol, ref plt) && (plt.State == PalletStatus.Invalid))
                                        {
                    ...
                    {
                        //Def.WriteLog("RunProcessRobotTransfer", "模组使能没开，运行状态，非同一干燥炉转移换腔");
                    }
                }
            }
            return false;
        ...
        private bool SearchOnloadPickPos(EventList searchEvent, ref PickPlacePos pick)
        {
            ...
            EventStatus statue = GetModuleEvent(runId, searchEvent, ref pickPltIdx);
            if ((EventStatus.Require != statue)
                ...
                case EventList.OnloadPickOKFullPallet:
                    {
                        ...
                        if (1 == this.placeFakePalletCnt)
                        {
                            if ((PalletStatus.OK == plt.State)
                            && (PalletStage.Onload == plt.Stage)
                            && !plt.IsEmpty() && !plt.HasFake())
                            {
                                pick.SetData(runId, TransferRobotStation.OnloadStation, 0, pickPltIdx, searchEvent);
                                return true;
                            }
        ...
        private bool SearchOnloadPlacePos(EventList searchEvent, ref PickPlacePos place)
        {
            ...
            // 信号事件状态不正确
            EventStatus state = GetModuleEvent(runId, searchEvent, ref placePltIdx);
        ...
        private bool SearchDryOvenPlacePos(RunID runId, ModDef searchMode, EventList searchEvent, ref PickPlacePos place)
        {
            ...
            // 信号事件状态不正确
            EventStatus status = GetModuleEvent(runId, searchEvent, ref placePltIdx);
            if ((EventStatus.Require != status))
            {
                return false;
            }
        ...
        private bool SearchOffloadPlacePos(EventList searchEvent, ref PickPlacePos place)
        {
            ...
            // 信号事件状态不正确
            EventStatus sta = GetModuleEvent(runId, searchEvent, ref placePltIdx);
            if (EventStatus.Require != sta)
            {
                return false;
            }
            switch (searchEvent)
        ...
        private bool PresetModuleEvent(PickPlacePos pos, EventStatus eventState)
        {
            ...
            {
                EventStatus state = GetModuleEvent(pos.runID, pos.stationEvent, ref eventPos);
                if (EventStatus.Require == state)
                {
        ...
        /// <param name="cavityIdx"></param>
        /// <returns></returns>
        private bool GetDryingOvenCavityTransferRecv(RunID runId, int cavityIdx)
        {
            return MachineCtrl.GetInstance().GetDryingOvenCavityTransferRecv(runId, cavityIdx);
        }
        ...
        /// 通过机器人站点获取模组
        /// </summary>
        public bool GetModuleByStation(TransferRobotStation station, ref RunID runID)
        {
            // 干燥炉
            if (station >= TransferRobotStation.DryOven_0 && station <= TransferRobotStation.DryOven_All)
            {
                runID = RunID.DryOven0 + (station - TransferRobotStation.DryOven_0);
                return true;
            }
            // 托盘缓存架
            ...
            // 人工操作平台
            else if (TransferRobotStation.ManualOperate == station)
            {
                runID = RunID.ManualOperate;
                return true;
            }
        ...
        /// <param name="pallet"></param>
        /// <returns></returns>
        private bool SetModulePallet(RunID runId, int pltIdx, Pallet pallet, bool place = false)
        {
            return MachineCtrl.GetInstance().SetModulePallet(runId, pltIdx, pallet, place);
        }
        ...
        public override void ManualResetEvent()
        {
            //查找调度机器人当前关联的模组
            LoadRunData();
            RunID runId = RunID.OnloadLine;
            if (GetModuleByStation(pickPos.station, ref runId))
            {
                MachineCtrl.GetInstance().GetModule(runId).ManualResetEvent();
            }
            if (GetModuleByStation(placePos.station, ref runId))
            {
                MachineCtrl.GetInstance().GetModule(runId).ManualResetEvent();
            }
