...
namespace Machine
{
    ...
    class RunProcessManualOperate : RunProcess
    {
                ...
                case AutoSteps.Auto_WaitWorkStart:
                    {
                        ...
                        // 人工操作请求
                        if((this.onloadTest || onloadRobot.onloadData.bufSignalBtnStat) 
                            || (this.offloadTest || onloadRobot.onloadData.bufSignalBtnStop))
                        {
                        ...
                        if((this.offloadTest || (PalletStatus.Invalid != this.Pallet[0].State))
                ...
                case AutoSteps.Auto_WaitManualOperateFinish:
                    {
                        ...
                        if(onloadRobot.onloadData.bufSignalBtnStat)
                        {
                        ...
                        else if(onloadRobot.onloadData.bufSignalBtnStop)
                        {
        ...
        public override bool PalletKeepFlat(int pltIdx, bool hasPlt, bool alarm = true)
        {
            ...
            if (hasPlt)
            {
                if ((!onloadRobot.onloadData.operateSignal[pltIdx, 0]
               || !onloadRobot.onloadData.operateSignal[pltIdx, 1]
               || !onloadRobot.onloadData.operateSignal[pltIdx, 2]))
                {
                    if (alarm)
                    {
                        ShowMessageBox((int)MsgID.Start, "夹具在位感应器信号异常", "请检查夹具在位感应器是否正常！！", MessageType.MsgWarning);
                    }
                    return false;
                }
            }
        ...
        public override void ManualResetEvent()
        {
            //查找调度机器人当前关联的模组
            LoadRunData();
            for (int i = (int)EventList.ManualPlaceNGEmptyPallet; i < (int)EventList.ManualPickPlaceEnd; i++)
            {
