...
namespace Machine
{
    ...
    class RunProcessPalletBuffer : RunProcess
    {
        ...
        /// 指定缓存一放NG夹具
        /// </summary>
        public bool IsBufferOnePlaceNgPallet;        
        ...
        public RunProcessPalletBuffer(int runId) : base(runId)
        {
            ...
            InsertVoidParameter("IsBufferOnePlaceNgPallet", "指定暂存架位一放NG夹具", "指定暂存架位一放NG夹具：TRUE启用，只有暂存架夹具位一可放NG夹具；FALSE禁用，所有夹具位可均放空夹具或NG夹具", this.IsBufferOnePlaceNgPallet, RecordType.RECORD_BOOL, ParameterLevel.PL_IDLE_ADMIN);
                ...
                case AutoSteps.Auto_WaitWorkStart:
                    {
                        ...
                        bool IsCanPlaceEmptyPtl = true;
                        bool IsCanPlaceNgPtl = true;
                        for (int idx = 0; idx < this.Pallet.Length; idx++)
                        {
                            ...
                            {
                                if (IsBufferOnePlaceNgPallet && 0 == idx)
                                {
                                    // 缓存架一放NG空夹具
                                    ...
                                    if ((EventStatus.Invalid == state) || (EventStatus.Finished == state))
                                    {
                                        ...
                                        Trace.WriteLine($"{modEvent}-{EventStatus.Require}-{idx}");
                                    }
                                }
                                else
                                {
                                    ...
                                    // 缓存架放OK满料夹具
                                    modEvent = EventList.PalletBufferPlaceOKFullPallet;
                                    state = GetEvent(this, modEvent);
                                    if ((EventStatus.Invalid == state) || (EventStatus.Finished == state))
                                    {
                                        SetEvent(this, modEvent, EventStatus.Require, idx);
                                    ...
                                    // 缓存架放OK满料带假电池夹具
                                    modEvent = EventList.PalletBufferPlaceOKFakeFullPallet;
                                    state = GetEvent(this, modEvent);
                                    if ((EventStatus.Invalid == state) || (EventStatus.Finished == state))
                                    {
                                        SetEvent(this, modEvent, EventStatus.Require, idx);
                            ...
                            if ((PalletStatus.OK == this.Pallet[idx].State) && this.Pallet[idx].IsEmpty())
                            {
                                modEvent = EventList.PalletBufferPickOKFullPallet;
                                state = GetEvent(this, modEvent);
                                if ((EventStatus.Invalid == state) || (EventStatus.Finished == state))
                                {
                                    SetEvent(this, modEvent, EventStatus.Require, idx);
                            ...
                            if ((PalletStatus.OK == this.Pallet[idx].State) && this.Pallet[idx].IsEmpty())
                            {
                                modEvent = EventList.PalletBufferPickOKFakeFullPallet;
                                state = GetEvent(this, modEvent);
                                if ((EventStatus.Invalid == state) || (EventStatus.Finished == state))
                                {
                                    SetEvent(this, modEvent, EventStatus.Require, idx);
        ...
        {
            this.IsBufferOnePlaceNgPallet = false;
        ...
        public override void ReadRelatedModule()
        {
            ...
            //// 待检测电池
            //strValue = iniStream.ReadString(strModule, "DetectBattery", "", Def.GetAbsPathName(Def.ModuleExCfg));
            //detectBattery = MachineCtrl.GetInstance().GetModule(strValue) as RunProDetectBattery;
            // 上料模组
        ...
        public override bool ReadParameter()
        {
            ...
            this.IsBufferOnePlaceNgPallet = ReadBoolParameter(this.RunModule, ("IsBufferOnePlaceNgPallet"), this.IsBufferOnePlaceNgPallet);
        ...
        public override bool PalletKeepFlat(int pltIdx, bool hasPlt, bool alarm = true)
        {
            ...
            if (hasPlt)
            {
                if ((!onloadRobot.onloadData.bufSignal[pltIdx, 0]
               || !onloadRobot.onloadData.bufSignal[pltIdx, 1]
               || !onloadRobot.onloadData.bufSignal[pltIdx, 2]))
                {
                    if (alarm)
                    {
                        ShowMessageBox((int)0, "夹具在位感应器信号异常", "请检查夹具在位感应器是否正常！！", HelperLibrary.MessageType.MsgWarning);
                    }
                    return false;
                }
            }
        ...
        public override void ManualResetEvent()
        {
            //查找调度机器人当前关联的模组
            LoadRunData();
            for (int i = (int)EventList.PalletBufferPlaceEmptyPallet; i < (int)EventList.PalletBufferPickPlaceEnd; i++)
            {
