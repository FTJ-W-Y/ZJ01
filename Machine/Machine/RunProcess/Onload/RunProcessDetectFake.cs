...
namespace Machine
{
    ...
    /// 待测假电池线体
    /// </summary>
    class RunProcessDetectFake : RunProcess
    {
        ...
        private int IBufferPosOut;              // 缓存位出
        ...
        public RunProcessDetectFake(int RunID) : base(RunID)
        {
                ...
                case AutoSteps.Auto_WaitWorkStart:
                    {
                        ...
                        // 有空位，发送放电池信号
                        EventStatus state = GetEvent(this, EventList.PlaceDetectBattery);
                        if (!PlacePosIsFull() && ((EventStatus.Invalid == state) || (EventStatus.Finished == state)))
                        {
                            SetEvent(this, EventList.PlaceDetectBattery, EventStatus.Require);
                        }
                        // 有空位，已响应
                        else if((EventStatus.Response == state) && !PlacePosIsFull())
                        {
                            if(PlaceSenserIsSafe(true))
                            {
                                ...
                                SetEvent(this, EventList.PlaceDetectBattery, EventStatus.Ready);                               
                            }
                        }
                               ...
                               || (EventStatus.Cancel == state)))
                        {
                            ...
                            else if(/*!PlacePosIsEmpty() &&*/ ManualButton(true) && !ManualButton(false))
                            {
                ...
                case AutoSteps.Auto_SendToBufferPos:
                    {
                        ...
                        {
                            if (EventStatus.Cancel == GetEvent(this, EventList.PlaceDetectBattery))
                            {
                                SetEvent(this, EventList.PlaceDetectBattery, EventStatus.Invalid);
                            }
