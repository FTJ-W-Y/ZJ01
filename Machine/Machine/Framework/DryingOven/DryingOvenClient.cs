    ...
    class DryingOvenClient : BaseThread
    {
            ...
            get
            {
                if (null == finsCmdAddr)
                {
                    finsCmdAddr = new DryOvenFinsCmd[(int)DryOvenCmd.End]
                    {
                        ...
                        new DryOvenFinsCmd(ZoneCode.DMWord, 3000, 10, 0, 0, 50),
                        //RunState,                   // 工作状态（读取）
                        new DryOvenFinsCmd(ZoneCode.DMWord, 3050, 10, 0, 0, 50),
                        //RunTemp,                    // 实时温度（读取）
                        new DryOvenFinsCmd(ZoneCode.DMWord, 3100, 160, 0, 0, 800),
                        //AlarmValue,                 // 报警值（温度、真空）（读取）
                        new DryOvenFinsCmd(ZoneCode.DMWord, 3900, 84, 0, 0, 420),
                        //AlarmState,                 // 报警状态（读取）
                        new DryOvenFinsCmd(ZoneCode.DMWord, 4320, 40, 0, 0, 200),
                        //GetParameter,               // 工艺参数（读）
                        new DryOvenFinsCmd(ZoneCode.DMWord, 4520, 50, 0, 0, 250),
                        //SystemInfo,                 // 干燥炉系统信息（读）
                        new DryOvenFinsCmd(ZoneCode.DMWord, 4820, 0, 0, 0, 20),

                        //SetParameter,               // 工艺参数（可读可写）
                        new DryOvenFinsCmd(ZoneCode.DMWord, 4520, 50, 0, 0, 25*2),
                        //WorkStartStop,              // 加热启动/停止（写入）
                        new DryOvenFinsCmd(ZoneCode.DMWord, 4770, 10, 0, 0, 1),
                        //DoorOpenClose,              // 炉门打开/关闭（写入）
                        new DryOvenFinsCmd(ZoneCode.DMWord, 4771, 10, 0, 0, 1),
                        //VacOpenClose,               // 真空打开/关闭（写入）
                        new DryOvenFinsCmd(ZoneCode.DMWord, 4772, 10, 0, 0, 1),
                        //BlowOpenClose,              // 破真空打开/关闭（写入）
                        new DryOvenFinsCmd(ZoneCode.DMWord, 4773, 10, 0, 0, 1),
                        //PressureOpenClose,          // 保压打开/关闭（写入）
                        new DryOvenFinsCmd(ZoneCode.DMWord, 4774, 10, 0, 0, 1),
                        //FaultReset,                 // 故障复位（写入）
                        new DryOvenFinsCmd(ZoneCode.DMWord, 4775, 10, 0, 0, 1),
                        //SetMcDoor,                  // 调度设备门禁（写入）
                        new DryOvenFinsCmd(ZoneCode.DMWord, 4821, 0, 0, 0, 1),
                        //AlarmReset,                 // 报警复位（写入）
                        new DryOvenFinsCmd(ZoneCode.DMWord, 4775, 10, 0, 0, 1),
                    };
                }
                        ...
                        switch(this.finsType)
                        {
                            ...
                            case FinsType.Udp:
                                ...
                                else
                                {
                                    ...
                                    {
                                        this.errCount++;
                                        WriteLog($"{this.finsUdp.GetIPInfo()}.DryingOvenClient.RunWhile() read {((DryOvenCmd)i).ToString()} fail.", HelperLibrary.LogType.Error);
                                    }
                                }
                                break;
                        }
                ...
                case DryOvenCmd.GetParameter:
                    {
                        ...
                        for(int cavityIdx = 0; cavityIdx < (int)OvenRowCol.MaxRow; cavityIdx++)
                        {
                            ...
                            cavityData.parameter.BStateSavePressureTime = BitConverter.ToUInt32(buffer, (idx += 4)); //B状态真空保压时间
                            ...
                            cavityData.parameter.AStateVacMaxValue = BitConverter.ToUInt32(buffer, (idx += 4));
                            cavityData.parameter.BStateVacMaxValue = BitConverter.ToUInt32(buffer, (idx += 4));
                            cavityData.parameter.TempDifferAlarmValue = BitConverter.ToUInt32(buffer, (idx += 4));
                            cavityData.parameter.AStateSavePressureTime = BitConverter.ToUInt32(buffer, (idx += 4)); //A状态真空保压时间
                        }
                        break;
                    }
        ...
        private bool DataToBuf(DryOvenCmd cmdID, int count, CavityData cavityData, ref byte[] sendData)
        {
            ...
            switch(cmdID)
            {
                case DryOvenCmd.SetParameter:
                    {
                        ...
                        {
                            buf = BitConverter.GetBytes(Convert.ToUInt32(para.SetTempValueEx * 100));
                            Array.Copy(buf, 0, sendData, idx, buf.Length);
                            idx += buf.Length;
                            buf = BitConverter.GetBytes(Convert.ToUInt32(para.TempUpperlimitEx * 100));
                            Array.Copy(buf, 0, sendData, idx, buf.Length);
                            idx += buf.Length;
                            buf = BitConverter.GetBytes(Convert.ToUInt32(para.TempLowerlimitEx * 100));
                            Array.Copy(buf, 0, sendData, idx, buf.Length);
                            idx += buf.Length;
                            buf = BitConverter.GetBytes(para.PreheatTimeEx);
                            Array.Copy(buf, 0, sendData, idx, buf.Length);
                            idx += buf.Length;
                            buf = BitConverter.GetBytes(para.VacHeatTimeEx);
                            Array.Copy(buf, 0, sendData, idx, buf.Length);
                            idx += buf.Length;
                            buf = BitConverter.GetBytes(para.OpenDoorBlowTimeEx);
                            Array.Copy(buf, 0, sendData, idx, buf.Length);
                            idx += buf.Length;
                            buf = BitConverter.GetBytes(para.OpenDoorVacPressureEx);
                            Array.Copy(buf, 0, sendData, idx, buf.Length);
                            idx += buf.Length;
                            buf = BitConverter.GetBytes(para.AStateVacTimeEx);
                            Array.Copy(buf, 0, sendData, idx, buf.Length);
                            idx += buf.Length;
                            buf = BitConverter.GetBytes(para.AStateVacPressureEx);
                            Array.Copy(buf, 0, sendData, idx, buf.Length);
                            idx += buf.Length;
                            buf = BitConverter.GetBytes(para.BStateVacTimeEx);
                            Array.Copy(buf, 0, sendData, idx, buf.Length);
                            idx += buf.Length;
                            buf = BitConverter.GetBytes(para.BStateVacPressureEx);
                            Array.Copy(buf, 0, sendData, idx, buf.Length);
                            idx += buf.Length;
                            buf = BitConverter.GetBytes(para.BStateBlowAirTimeEx);
                            Array.Copy(buf, 0, sendData, idx, buf.Length);
                            idx += buf.Length;
                            buf = BitConverter.GetBytes(para.BStateBlowAirPressureEx);
                            Array.Copy(buf, 0, sendData, idx, buf.Length);
                            idx += buf.Length;
                            buf = BitConverter.GetBytes(para.BStateBlowAirKeepTimeEx);
                            Array.Copy(buf, 0, sendData, idx, buf.Length);
                            idx += buf.Length;
                            buf = BitConverter.GetBytes(para.BreathTimeIntervalEx);
                            Array.Copy(buf, 0, sendData, idx, buf.Length);
                            idx += buf.Length;
                            buf = BitConverter.GetBytes(para.BreathCycleTimesEx);
                            Array.Copy(buf, 0, sendData, idx, buf.Length);
                            idx += buf.Length;
                            buf = BitConverter.GetBytes(para.HeatPlateEx);
                            Array.Copy(buf, 0, sendData, idx, buf.Length);
                            idx += buf.Length;
                            buf = BitConverter.GetBytes(para.MaxNGHeatPlateEx);
                            Array.Copy(buf, 0, sendData, idx, buf.Length);
                            idx += buf.Length;
                            buf = BitConverter.GetBytes(para.HeatPreVacTimeEx);
                            Array.Copy(buf, 0, sendData, idx, buf.Length);
                            idx += buf.Length;
                            buf = BitConverter.GetBytes(para.HeatPreBlowEx);
                            Array.Copy(buf, 0, sendData, idx, buf.Length);
                            idx += buf.Length;
                            buf = BitConverter.GetBytes(para.VacMinValueEx);
                            Array.Copy(buf, 0, sendData, idx, buf.Length);
                            idx += buf.Length;
                            buf = BitConverter.GetBytes(para.VacMaxValueEx);
                            Array.Copy(buf, 0, sendData, idx, buf.Length);
                            idx += buf.Length;
                            buf = BitConverter.GetBytes(para.TempDifferAlarmValueEx);
                            Array.Copy(buf, 0, sendData, idx, buf.Length);
                            idx += buf.Length;
                        }
                        else
                        {
                            buf = BitConverter.GetBytes(Convert.ToUInt32(para.SetTempValue * 100));
                            Array.Copy(buf, 0, sendData, idx, buf.Length);
                            idx += buf.Length;
                            ...
                            Array.Copy(buf, 0, sendData, idx, buf.Length);
                            idx += buf.Length;
                            buf = BitConverter.GetBytes(para.BStateSavePressureTime);  //B状态真空保压时间  add 0709
                            Array.Copy(buf, 0, sendData, idx, buf.Length);
                            idx += buf.Length;
                            ...
                            Array.Copy(buf, 0, sendData, idx, buf.Length);
                            idx += buf.Length;
                            buf = BitConverter.GetBytes(para.AStateVacMaxValue);
                            Array.Copy(buf, 0, sendData, idx, buf.Length);
                            idx += buf.Length;
                            buf = BitConverter.GetBytes(para.BStateVacMaxValue);
                            Array.Copy(buf, 0, sendData, idx, buf.Length);
                            idx += buf.Length;
                            buf = BitConverter.GetBytes(para.TempDifferAlarmValue);
                            Array.Copy(buf, 0, sendData, idx, buf.Length);
                            idx += buf.Length;
                            buf = BitConverter.GetBytes(para.AStateSavePressureTime);  //A状态真空保压时间   add 0709
                            Array.Copy(buf, 0, sendData, idx, buf.Length);
                            idx += buf.Length;
                ...
                case DryOvenCmd.AlarmReset:
                    {
                        result = true;
                        byte[] buf = BitConverter.GetBytes(cavityData.alarmState);
                        ByteCodec(buf, 0, count * 2, CodecMode.bit16_21);
                        Array.Copy(buf, 0, sendData, 0, buf.Length);
                        break;
                    }
        ...
        public bool SetDryOvenData(DryOvenCmd cmdID, int cavityIdx, CavityData cavityData)
        {
            ...
            if(DataToBuf(cmdID, cmd.count, cavityData, ref sendBuffer))
            {
                switch(this.finsType)
                {
                    ...
                    case FinsType.Udp:
                        return this.finsUdp.WriteWords(cmd.zone, (short)(cmd.wordAddr + cavityIdx * cmd.wordInterval), cmd.bitAddr, cmd.count, sendBuffer);
                }
            }
            return false;
        }
