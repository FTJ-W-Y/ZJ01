...
namespace Machine
{
    ...
    class RobotSocketGeneral : BaseThread
    {
        ...
        /// <returns></returns>
        public bool Send(int[] cmd, OptMode mode, ref string errMsg)
        {
            if (SendToRobot(cmd, mode, ref errMsg))
            {
                DateTime time = DateTime.Now;
                while((DateTime.Now - time).TotalSeconds < 10.0) //10s超时
                {
                    if(this.recvFinish)
                    {
        ...
        /// <returns></returns>
        public bool SendAndWait(int[] cmd, ref int[] recvBuf, OptMode mode, ref string errMsg, double delayTime = 30.0)
        {
            if (!Send(cmd, mode, ref errMsg))
            {
                return false;
            }
        ...
        /// <param name="cmd"></param>
        /// <returns></returns>
        private bool SendToRobot(int[] cmd, OptMode mode, ref string errMsg)
        {
            this.recvFinish = false;
            string strCmd = GetRobotCmd(cmd);
            OutputLog(true, strCmd, mode);
            return this.client.Send(strCmd, ref errMsg);
        }
