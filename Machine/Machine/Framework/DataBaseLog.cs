...
namespace Machine
{
    public static class DataBaseLog
    {
        ...
        public static bool GetPalletList(int _formulaID, int _motorID, string _startTime, string _endTime, ref DataTable alarmTable)
        {
            if (!isOpen)
            {
                return false;
            ...
            try
            {
                lock (dbLock)
                {
                    string sql = string.Format("SELECT * FROM PalletTable");
                    //string info = "";
                    //if (_motorID < 0)
                    //{
                    //    info = string.Format(" WHERE {0} = '{1}' AND {2} BETWEEN '{3}' AND '{4}'"
