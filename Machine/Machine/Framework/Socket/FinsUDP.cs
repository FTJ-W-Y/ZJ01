...
namespace Machine
{
    ...
    public class FinsUDP : BaseThread
    {
        ...
        private bool CheckError(byte[] data)
        {
            ...
            //if ((data[(int)FinsFrame.RequireZone] != 0) || (data[(int)FinsFrame.RequireZone + 1] != 0)) //wjj 220504
            {
                return false;
            }
            return true;
        ...
        public bool ReadWords(ZoneCode zCode, short wordAddr, short bitAddr, short count, ref byte[] recvBuffer)
        {
            ...
            lock (this.rwLock)
            {
                if (SendAndWait(sBuf, sBuf.Length, ref rBuf, rBuf.Length))
                {
                    if (CheckError(rBuf))
                    {
                        int cc = (int)FinsFrame.ResponseEnd;
                        Array.Copy(rBuf, (int)FinsFrame.ResponseEnd, recvBuffer, 0, (count * 2));

                    }
                    if (MachineCtrl.GetInstance().WritePlcLog)
                    {
                        string szSendHx = "";
                        ...
                        Def.WriteLog("FinsUDP读数据", $"发送指令：{szSendHx}", HelperLibrary.LogType.Information);
                        ...
                        Def.WriteLog("FinsUDP读数据", $"接收指令：{szRecvHx}", HelperLibrary.LogType.Information);
                    }
                    return true;
                }
            }
                ...
                if (SendAndWait(sBuf, sBuf.Length, ref rBuf, rBuf.Length))
                {
                    if (CheckError(rBuf))
                    {
                        //if (MachineCtrl.GetInstance().WritePlcLog)
                        {
                            ...
                            Def.WriteLog("FinsUDP写数据", $"发送指令：{szSendHx}", HelperLibrary.LogType.Information);
                            ...
                            Def.WriteLog("FinsUDP写数据", $"接收指令：{szRecvHx}", HelperLibrary.LogType.Information);
                        }
                        return true;
                    }
                }
