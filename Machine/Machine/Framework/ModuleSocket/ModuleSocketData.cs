...
namespace Machine
{
    ...
    public class ModuleSocketData
    {
        ...
        // 模组所在设备数据
        public short machineID;                                    // 设备ID：MachineCtrl.MachineID，只初始化一次
        public int machineState;                                 // 设备状态：SystemControlLibrary.MCState枚举值
        public bool[] safeDoor;                                  // 设备安全门状态：true已打开，false已关闭
        public bool[] safeDoorStop;                              // 设备安全门急停：true触发，false取消
        public short onloadCount;                                // 上料计数
        public short onScanNGCount;                              // 扫码NG计数
        public short onScan1NGCount;                             // 扫码枪1-NG计数
        public short onScan2NGCount;                             // 扫码枪2-NG计数
        public short onScan3NGCount;                             // 扫码枪3-NG计数
        public short onScan4NGCount;                             // 扫码枪4-NG计数
        public short offloadCount;                               // 下料计数
        public short bakedNGCount;                               // 烘烤NG计数
        ...
        public Dictionary<RunID, bool[]> cavityTransferRecv;     // 干燥炉腔体转移接收：true启用，false禁用
        ...
        public void CreateData()
        {
            lock(this.dataLock)
            {
                ...
                {
                    this.safeDoor[i] = false;
                }
                this.safeDoorStop = new bool[(int)SystemIO.SafeDoorIO];
                for (int i = 0; i < this.safeDoorStop.Length; i++)
                {
                    this.safeDoorStop[i] = false;
                }
                ...
                this.cavityTransferRecv = new Dictionary<RunID, bool[]>();
                    ...
                    for(int i = 0; i < runId.Length; i++)
                    {
                        ...
                        if(this.cavityState.ContainsKey(runId[i]))
                        {
                            ...
                            for(int j = 0; j < this.cavityState[runId[i]].Length; j++)
                            {
                                ...
                                this.cavityTransferRecv[runId[i]][j] = false;
