    ...
    sealed class ModuleServer : ServerSocket
    {
        ...
        private bool GetModuleData(uint dataType, ref ModuleSocketData socketData)
        {
            lock(socketData.dataLock)
            {
                ...
                int ioInput = -1;
                socketData.machineID = mc.MachineID;
                socketData.machineState = (int)mc.RunsCtrl.GetMCState();
                for(int i = 0; i < socketData.safeDoor.Length; i++)
                {
                    socketData.safeDoor[i] = mc.SafeDoorOpenState(i, ref ioInput);
                    //if (socketData.safeDoor[i])
                    //{
                    //    Def.WriteLog("GetModuleData ", string.Format("门状态感应器{0:d4}为ON", ioInput));
                    //}
                }
                for (int i = 0; i < socketData.safeDoorStop.Length; i++)
                {
                    socketData.safeDoorStop[i] = mc.SafeDoorIsStop(i);
                }
                        ...
                        default:
                            {
                                if((idList[i] >= RunID.DryOven0) && (idList[i] < RunID.DryOvenALL))
                                {
                                    ...
                                    if(null != oven)
                                    {
                                        ...
                                        socketData.cavityTransferRecv[idList[i]] = oven.CavityTransferRecv;
