...
namespace Machine
{
    ...
    public class Pallet
    {
        ...
        public DateTime LogonCheckDate;                  // 夹具入站时间
        public DateTime StartDate;                  // 加热开始时间
        public DateTime EndDate;                    // 加热结束时间
        public short BakingCnts;                    // 夹具累计烘烤出炉次数
        public uint batCount;                      // 夹具当前电池总数
        public bool OutBak;                  // 夹具状态
        ...
        public void Release()
        {
            lock(palletLock)
            {
                ...
                this.SrcRow = -1;
                this.SrcCol = -1;
                this.LogonCheckDate = DateTime.MinValue;
        ...
        public void Copy(Pallet srcPallet)
        {
            lock(palletLock)
            {
                ...
                this.LogonCheckDate= srcPallet.LogonCheckDate;
                this.StartDate = srcPallet.StartDate;
                this.EndDate = srcPallet.EndDate;
                this.BakingCnts = srcPallet.BakingCnts;
                for (int row = 0; row < this.MaxRow; row++)
                {
                    for(int col = 0; col < this.MaxCol; col++)
                    {
        ...
        public int PalletBatCnt()
        {
            lock (palletLock)
            {
                ...
                for (int row = 0; row < this.MaxRow; row++)
                {
                    for (int col = 0; col < this.MaxCol; col++)
                    {
                        if (BatteryStatus.OK == this.Battery[row, col].Type)
                        {
                            cnt++;
                        }
                    }
        ...
        public bool HasFakeFlag()
        {
            lock (palletLock)
            {
                for (int row = 0; row < this.MaxRow; row++)
                {
                    for (int col = 0; col < this.MaxCol; col++)
                    {
                        if ((BatteryStatus.FakeTag == this.Battery[row, col].Type))
                        {
                            return true;
                        }
                    }
